# 02 OpenAI 社区：DNA Template 角色一致性技巧
...
- **DNA Template 是社区发现的角色一致性核心技巧**：在每次生成新场景前，先发送一段完整的角色描述模板（DNA），锁定角色的所有视觉特征、风格和基调，然后再给出具体场景指令。
- **模板必须包含的核心要素**：
...
- **区分角色差异的标记物**：对于多个相似角色（如三胞胎），为每个角色分配独特的配饰（丝带、吊坠、日记本等），以区分身份。
...
- **多轮生成（Multi-Turn）策略**：配合 DNA Template，在一个对话中逐个生成场景，每个场景前都重发 DNA，确保模型对角色的理解不漂移。

## 原文摘要

OpenAI 社区用户 june0407 提出了对角色一致性和风格锁定的迫切需求，社区成员 polepole 提供了 DNA Template 解决方案。该模板本质上是一个完整的角色设定文档，包含格式、风格、场景环境、角色外貌（从头到脚的详细描述）、情绪基调、渲染参数和构图规范。核心思路是：在 ChatGPT 的多轮对话中，每次生成新场景前都先发送 DNA Template，让模型"重新校准"对角色的理解，从而减少因上下文累积导致的特征漂移。该方案特别适用于多角色叙事（如三胞胎姐妹）和长篇视觉故事。
