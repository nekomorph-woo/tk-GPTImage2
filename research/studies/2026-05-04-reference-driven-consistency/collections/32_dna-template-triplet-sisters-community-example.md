...
这是 polepole 在 OpenAI 社区提供的完整 DNA Template 实例。它展示了 DNA Template 的标准结构：(1) 格式和比例锁定（3:2 watercolor anime）；(2) 项目概述和主题；(3) 完整的环境设定（沿海村庄、樱花树、灯笼）；(4) 三个角色的完全相同外观描述 + 差异化标记物（丝带/吊坠/日记本）；(5) 渲染参数（光线、色调、构图）；(6) 显式一致性声明（"must remain perfectly consistent in every image"）。该模板适用于在同一对话中生成多个场景，每次生成前重发此模板。
