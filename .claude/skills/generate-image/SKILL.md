---
name: generate-image
description: 生成单张图片、运行批量实验、Codex CLI 生图（无需 API Key）。Use when 用户要求生成图片、批量测试提示词、ChatGPT 生图、Codex 生图、"帮我生成..."、"批量..."、"用 Codex 生成..."。
---
...
└── batch.mjs    ← 批量编排层，通过 --mode api|codex 动态导入生图器
...
# 批量（从模板复制后运行）
cp .claude/skills/generate-image/references/starter-template.json gen/experiments/my-exp.json
node .claude/skills/generate-image/scripts/batch.mjs --matrix gen/experiments/my-exp.json
```

完整命令、参数表、实验矩阵说明见 [references/commands.md](references/commands.md)。
...
2. 提示词遵循框架，不堆叠模糊质量词
3. 批量实验一次只改一个变量
4. 中文海报精确标题放引号内，声明"不要额外随机文字"
5. Codex 模式固定输出 1254x1254，`--size` 是后处理缩放
