...
├── iwanna-image-prompt/references/genre-matching.md ← 流派匹配 + 提示词构造
...
**提示词构造**：参考 `iwanna-image-prompt/references/genre-matching.md` 的核心公式（格式 → 主体 → 构图 → 风格 → 光线 → 材质 → 色彩 → 文字 → 质量约束）。
...
| 路径 | 用途 |
|------|------|
| `iwanna-image-prompt/references/genre-matching.md` | 流派匹配 + 提示词构造公式 |
| `prompts/REVIEW_RUBRIC.md` | 7 维评分标准 |
