# #054

- **来源**: [@Salmaaboukarr](https://x.com/Salmaaboukarr/status/2047008188285792724)
- **采集时间**: 2026-05-03
- **Prompt 位置**: 主推文

## Prompt

```
Assemble the dieline into a flawless 3D box with accurate panels, clean folds, undistorted type, and artwork preserved exactly. shoot it upright at a refined ¾ angle in a minimal premium studio setting with a soft neutral background, diffused light, subtle shadows, no props, true colours, matte paperboard texture, and realistic editorial detail
```

## 用途

将包装模切线（dieline）转换为逼真 3D 包装盒渲染，精确面板+干净折叠+不变形文字+ artwork 原样保留，3/4 角度直立拍摄，极简高端棚拍设置+柔和中性背景+漫射光+微妙阴影+哑光纸板纹理+真实编辑细节，image-edit 适用。

## 标签

packaging, dieline, 3D-box, product-rendering, studio-photography, matte, paperboard, 3:4-angle, editorial, image-edit

## 图片样例

- ![样例 1](https://pbs.twimg.com/media/HGhuxSqXkAAi_Jt?format=jpg&name=small)
- ![样例 2](https://pbs.twimg.com/media/HGhuzLQW4AArkzd?format=jpg&name=small)

## 原始文本

you can now bring your dielines to life w/GPT image 2

prompt:
Assemble the dieline into a flawless 3D box...
```
file_path