...
| 依赖 | 用途 | 涉及技能 |
|------|------|----------|
| **bb-browser** | 浏览器自动化，X 平台 prompt 收集 | `/collect-image-2-prompt` |
| **Python 3** | 去重检查、运行时状态管理等辅助脚本 | `/collect-image-2-prompt` |
| **X 账号登录** | 在浏览器中登录 x.com | `/collect-image-2-prompt` |
...
├── collected/                      # 从外部平台收集的结构化 prompt（79+ 条）
